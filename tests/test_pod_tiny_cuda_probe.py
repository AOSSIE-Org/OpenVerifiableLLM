...
@pytest.mark.parametrize('fault',[None,'record-fails','replay-fails','resume-mismatch','incomplete-replay','wrong-record-parent','launcher-tamper'])
def test_bounded_fresh_process_probe_preserves_failures(tmp_path,fault):
    ...
    def execute(argv,**kwargs):
        calls.append(argv);assert argv[:3]==[sys.executable,'-I','-S'] and kwargs['check'] and kwargs['timeout']>0
    ...
    if fault:
        with pytest.raises((ValueError,RuntimeError)):m.probe(*args,execute=execute)
