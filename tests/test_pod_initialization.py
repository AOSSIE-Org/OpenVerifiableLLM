...
SCRIPT=Path(__file__).parents[1]/'scripts/pod_initialization.py'


def launch(tmp_path,*,change=None,action='record'):
...
''')
    ...
    args=[action,'--warmup-updates','4','--output',str(tmp_path/'numerical')]
...
def test_single_selected_audit_receives_exact_action_and_activity_and_never_retries(tmp_path):
    ...
    assert called['module']=='ovl_pipeline.initialization' and called['arguments'][:3]==['record','--warmup-updates','4']
...
def test_verify_selects_only_initializer_in_a_real_isolated_child(tmp_path):
    result,control,_,_=launch(tmp_path,action='verify')
    assert result.returncode==0,result.stderr
    called=json.loads((control/'audit/called.json').read_text())
    assert called['module']=='ovl_pipeline.initialization' and called['arguments'][0]=='verify'


def test_pilot_replay_is_not_an_initialization_action(tmp_path):
    result,control,_,_=launch(tmp_path,action='replay')
    assert result.returncode!=0 and not (control/'audit/called.json').exists()
