...
e=read_json(out/'endpoint-observation.json',canonical_required=False)
...
profile={'schema':'ovl.pod-ssh-profile.v1','pod_id':e['id'],'host':endpoint['host'],'port':endpoint['port'],'user':'root','remote_root':'/workspace/ovllm/feasibility-pilot-v1','endpoint_observation_sha256':file_hash(out/'endpoint-observation.json'),'known_hosts_sha256':file_hash(known),'host_key_trust':'operator-pinned-TOFU'}
...
"""
u=Path.home()/'.config/systemd/user/ovllm-sustained-feasibility-v1-workload.service';assert not u.exists();u.write_text(unit);(out/'workload.service').write_text(unit)
