...
base=Path('.ovllm-cache/5090-pilot-plan-dry-v1');base.mkdir(exist_ok=False)
...
v.update(attempt_id='ovllm-explicit-local-final-pilot-dry-'+'0'*32,maximum_seconds=23400,checkpoint_grace_seconds=1800,
         spent_usd='2.408749',outstanding_usd='19.801738',reserved_remaining_usd='58.5',allowance_usd='8.5')
...
select=runpy.run_path('.ovllm-cache/select_5090_pilot_v1.py')['select']
plan=select(base,profile,r,read_json(base/'inputs/prepared-plan.json'))
assert [s['name'] for s in plan['stages']]==['setup','prepared-inputs','cuda-record','cuda-replay','cuda-resume','wikipedia-record','wikipedia-replay','conversation-record','conversation-replay']
negatives=[]
...
result={'schema':'ovl.final-pilot-plan-check.v1','result':'PASS','scope':'Purely local actual validator/arithmetic with historical quote and explicit fake endpoint; NOT provisioning or production admission',
        ...
        'selected_reallocation':{'remaining_mandatory_before_usd':'67','pilot_allowance_usd':'8.5','remaining_mandatory_after_usd':'58.5','scope':'Candidate reallocation within original aggregate cap; actual measured full-run forecast still mandatory before production.'},
        ...
        'checks':negatives,'builder_sha256':file_hash(Path('.ovllm-cache/select_5090_pilot_v1.py'))}
