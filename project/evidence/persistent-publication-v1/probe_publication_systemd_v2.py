...
ROOT=Path.cwd();out=ROOT/'.ovllm-cache/publisher-systemd-probe-v2';out.mkdir(exist_ok=False)
...
for mode in ('timeout','failure','success-orphan','success-clean'):
 ...
 try:
  ...
  if mode in ('timeout','success-orphan'):assert 'Result=timeout' in final
