...
intent=read_json(base/'range-download-intent-v2.json')
verify_inventory(root,intent['source_files']);assert file_hash(Path(__file__))==intent['driver_sha256']
assert read_json(base/'public-stream-download-v1/failure.json')==intent['predecessor_failure']
...
output=base/'public-range-download-v2';assert not output.exists()
with (base/'range-download-launch-v2.json').open('xb') as f:
...
result=verify(base/'plan.json',intent['plan_sha256'],intent['revision'],root/'.ovllm-cache/production-preparation-v1',output)
