...
if __name__=='__main__':
    base=Path('.ovllm-cache/sustained-candidate-v1')
    # Refuse until the full public prepared download has actually passed.
    check=read_json(base/'verified-prepared-public-parent.json')
    assert check['result']=='PASS' and check['schema']=='ovl.prepared-stream-download-verification.v1'
    prepared=read_json(base/'inputs/prepared-plan.json')
    assert check['revision']==prepared['revision'] and check['repo']==prepared['repo']
    assert all(f in check['files'] for f in prepared['files'])
