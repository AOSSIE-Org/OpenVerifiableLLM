...
root=Path.cwd();out=root/'.ovllm-cache/sustained-candidate-v2'
raw=(root/'project/evidence/provider-survey/gpu-catalog-candidate-cycle-v2.json').read_text()
cat=json.loads(raw);gpu=next(g for g in cat['gpus'] if g['id']=='NVIDIA RTX 2000 Ada Generation');assert gpu['availability']!='NONE' and gpu['price']['secure']==.24
...
observed=read_json(root/'project/evidence/provider-survey/gpu-catalog-candidate-cycle-v2-observation.json',canonical_required=False)['observed_epoch']
baseline=account();now=int(time.time());assert now-observed<=600
quote={'schema':'ovl.rental-quote.v1','observed_epoch':observed,'catalog_response':raw,'catalog_response_sha256':file_hash(root/'project/evidence/provider-survey/gpu-catalog-candidate-cycle-v2.json'),'selected_gpu':{'id':gpu['id'],'secure':True,'secure_hourly_usd':'0.24'},'storage_source':'https://docs.runpod.io/pods/pricing','storage_page_sha256':storage['sha256'],'storage_observed_epoch':storage['observed_epoch'],'container_gb_month_usd':'0.10','volume_gb_month_upper_usd':'0.20','monthly_hours':672,'rate_margin_percent':125}
rate=format(((Decimal('.24')+Decimal(80)*Decimal('.10')/672)*Decimal('1.25')).quantize(Decimal('.000001'),rounding=ROUND_CEILING),'f')
plan=rental_plan({'schema':'ovl.rental-budget-input.v2','attempt_id':'ovllm-candidate-cycle-'+uuid.uuid4().hex,'now_epoch':now,'spent_usd':'0.835711','outstanding_usd':'3.025030','reserved_remaining_usd':'85.0','allowance_usd':'1.0','hourly_upper_usd':rate,'quote_sha256':digest(quote),'maximum_seconds':10800,'checkpoint_grace_seconds':1500,'billing_slack_seconds':300,'external_termination_grace_seconds':120,'authorization_sha256':AUTHORIZATION_SHA256})
...
'''
 p=units/f'ovllm-sustained-candidate-v2-{name}.service';assert not p.exists();p.write_text(text);(out/(name+'.service')).write_text(text)
