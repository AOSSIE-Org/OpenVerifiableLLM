...
old=root/'.ovllm-cache/sustained-feasibility-v1'
for owner in ('controller','watchdog'):
 ...
 state=__import__('subprocess').run(['systemctl','--user','is-active','ovllm-sustained-feasibility-v1-'+owner+'.service'],capture_output=True,text=True)
...
observations=root/'.ovllm-cache/feasibility-v2-admission'
...
assert allocation['outstanding_usd']=='11.744764' and allocation['spent_usd']=='0.847606'
assert allocation['reserved_remaining_usd']=='72.0' and allocation['allowance_usd']=='5.0'
...
# Keep the complete reconstruction remainder and 10GiB pilot/index headroom,
...
assert disk.free-max(0,prepared_bytes-reconstructed)-10*1024**3>=disk.total//5
...
for name in ('input-inventory.json','preliminary-selection.json','qualification-overlap-admission.json'):
...
'''
 p=units/f'ovllm-sustained-feasibility-v2-{name}.service';assert not p.exists();p.write_text(text);(out/(name+'.service')).write_text(text)
