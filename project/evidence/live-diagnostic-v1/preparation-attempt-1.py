...
root=Path.cwd();out=root/'.ovllm-cache/live-diagnostic-v1'
raw=(root/'project/evidence/provider-survey/gpu-catalog-20260918T0924.json').read_text()
...
storage=read_json(root/'project/evidence/provider-survey/storage-pricing-20260918T0357.json')
# Retained MCP observation time, not preparation time.
observed=int((root/'project/evidence/provider-survey/gpu-catalog-20260918T0924.json').stat().st_mtime)
baseline=account();now=int(time.time());assert now-observed<=600
quote={'schema':'ovl.rental-quote.v1','observed_epoch':observed,'catalog_response':raw,'catalog_response_sha256':file_hash(root/'project/evidence/provider-survey/gpu-catalog-20260918T0924.json'),'selected_gpu':{'id':gpu['id'],'secure':True,'secure_hourly_usd':'0.74'},'storage_source':'https://docs.runpod.io/pods/pricing','storage_page_sha256':storage['sha256'],'storage_observed_epoch':storage['observed_epoch'],'container_gb_month_usd':'0.10','volume_gb_month_upper_usd':'0.20','monthly_hours':672,'rate_margin_percent':125}
rate=format(((Decimal('.74')+Decimal(40)*Decimal('.10')/672)*Decimal('1.25')).quantize(Decimal('.000001'),rounding=ROUND_CEILING),'f')
plan=rental_plan({'schema':'ovl.rental-budget-input.v2','attempt_id':'ovllm-diagnostic-'+uuid.uuid4().hex,'now_epoch':now,'spent_usd':'0.068185','outstanding_usd':'0.15','reserved_remaining_usd':'88','allowance_usd':'1','hourly_upper_usd':rate,'quote_sha256':digest(quote),'maximum_seconds':900,'checkpoint_grace_seconds':300,'billing_slack_seconds':300,'external_termination_grace_seconds':120,'authorization_sha256':AUTHORIZATION_SHA256})
...
rental={'schema':'ovl.rental-controller-intent.v1','watchdog_intent':w,'quote':quote,'payload':{**payload,'cloudType':'SECURE','gpuTypeId':gpu['id'],'minVcpuCount':4,'minMemoryInGb':16,'dockerArgs':'','startSsh':True,'startJupyter':False,'ports':'22/tcp'}}
...
'''
 p=units/f'ovllm-live-diagnostic-v1-{name}.service';assert not p.exists();p.write_text(text);(out/(name+'.service')).write_text(text)
