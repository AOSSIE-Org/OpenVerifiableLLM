...
def select(base,profile,rental,prepared_plan):
    ...
    for phase,interval in [('wikipedia',8192),('conversation',293)]:
      ...
      for mode in ('record','replay'):
        ...
        add(name,job,2100,1200 if phase=='wikipedia' else 1650,1024**3 if phase=='wikipedia' else 10*1024**3,parent_stage=parent,
    ...
    plan={'schema':'ovl.sustained-pilot-plan.v1','rental_intent_sha256':digest(rental),'profile_sha256':digest(profile),
        'worker_sha256':file_hash(Path('scripts/pod_job_worker.py')),'timing':{
            ...
            'basis':'Final candidate only: both real full streams,600second minimum records and complete sequential replays.2100second phase ceilings cover measured750second Wiki setup,600second numerical timing and actual bounded checkpoint deliveries. Each complete checkpoint waits for actual off-pod safe-state ACK under one fixed<=420second copy deadline; those waits are included in timed work except initial setup. Pilot intervals8192Wiki/293chat cover selected36160primary/9040recovery production density including final tails. Terminal limits count all logical bytes and actual rehashed uncached content before transfer. Floors1MiB/s transfer and100MiB/s hashing, ten hash passes plus30seconds, preserved. All four phase and export budgets plus setup/upload fit the original5hour rental with1800second graceful reserve, unchanged1800second durable-export age,300second progress age, and100USD/90USD/10USD caps. No production admission.'},
