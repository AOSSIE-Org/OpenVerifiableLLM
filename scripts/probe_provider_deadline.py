...
def request(operation, variables=None):
    ...
    try:
        ...
        raise Refused('provider request failed: HTTP status '+str(e.code)) from None
    except Exception as e:
        raise Refused('provider request failed: '+type(e).__name__) from None
...
def make_intent(now, initial, quote, image):
    ...
    plan=rental_plan({'schema':'ovl.rental-budget-input.v1','attempt_id':attempt,'now_epoch':now,
        'spent_usd':'0','outstanding_usd':'0','reserved_remaining_usd':'89.75','allowance_usd':'0.25',
    ...
    return {'schema':'ovl.deadline-probe-intent.v1','scope':'operational guard probe only; no training',
        ...
        'fallback_terminate_epoch':deadline+120,'quote':quote,'training_admission':'NOT_RUN'}
...
def run(directory, quote, image):
    with Journal(directory).lease() as journal:
        ...
        if first:
            ...
            initial=account();intent=make_intent(int(time.time()),initial,quote,image)
            ...
            try:
                ...
                try:journal.append('failure',{'stage':'creation','error_type':type(e).__name__,'reissue':'FORBIDDEN'})
                except Exception:
        ...
        while True:
            try:
                obs=account();pod=match_pod(intent,obs,known)
                ...
                try:journal.append('failure',{'stage':'observation-or-teardown','error_type':type(e).__name__})
                except Exception:pass
            time.sleep(20)
...
def main():
    ...
    if a.dry_run:
        observation=account();intent=make_intent(int(time.time()),observation,read_json(a.quote,canonical_required=False),a.image)
        print(json.dumps({'result':'DRY_RUN_NO_MUTATION','intent':intent}));return
    # This probe is currently fixed to a zero-spend project baseline. Do not reuse
    # after any paid attempt; general billing reconciliation must replace that gate.
    if os.environ.get('OVL_SYSTEMD_DEADLINE_PROBE')!='1':p.error('launch only through prepared restartable service')
    expected=Path(__file__).resolve().parents[1]/'.ovllm-cache/provider-deadline-probe-v1'
    if a.directory.resolve()!=expected:p.error('single authorized probe journal path required; no new attempt directories')
    run(a.directory,read_json(a.quote,canonical_required=False),a.image)
