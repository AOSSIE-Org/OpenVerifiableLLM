#!/usr/bin/env python3
"""One audited development pilot process under the selected worker deadline.
...
def run(setup_script,setup_sha256,config,config_sha256,inputs,runtime,control,deadline,arguments):
    ...
    if not arguments or arguments[0] not in ('record','replay'):raise ValueError('one explicit pilot action required')
    ...
    spec=importlib.util.spec_from_file_location('selected_pilot_audit',setup_script)
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
    result=module.audited(config,config_sha256,inputs,runtime,control/'audit','ovl_pipeline.gpu_pilot',arguments)
...
def main():
    ...
    except Exception as error:p.exit(1,'sustained pilot refused: '+type(error).__name__+'; preserve all partial outputs\n')
